...
const TraceabilityGraph: React.FC<{ 
  ...
  useEffect(() => {
    ...
    const node = g.append("g")
      ...
      .on("mouseover", (event, d) => {
        setHoverNode(d);
        // Highlight logic
        d3.select(event.currentTarget).attr("stroke", "#1f2937").attr("stroke-width", 2).attr("r", d.r * 1.5);
      })
      .on("mouseout", (event, d) => {
        setHoverNode(null);
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col lg:flex-row">
      <div className="flex-1 relative h-[600px] bg-gray-50" ref={wrapperRef}>
        <svg ref={svgRef} className="w-full h-full block touch-none"></svg>
        
        {/* Graph Controls overlay */}
        <div className="absolute top-4 left-4 flex flex-col gap-2">
          <div className="bg-white/90 backdrop-blur p-2 rounded-lg shadow-sm border border-gray-200 text-xs">
            <div className="font-bold text-gray-700 mb-2">Graph Layout</div>
        ...
        {hoverNode && (
          <div 
             className="absolute bg-gray-800 text-white text-xs px-3 py-2 rounded shadow-xl pointer-events-none z-10"
             style={{ left: 10, bottom: 10 }}
          >
             <div className="font-bold">{hoverNode.id}</div>
             <div className="text-gray-300">{hoverNode.name}</div>
          </div>
        )}
      </div>

      {/* Side Metrics Panel */}
      <div className="w-full lg:w-72 border-t lg:border-t-0 lg:border-l border-gray-200 bg-white p-6 flex flex-col">
         ...
         <div className="space-y-6">
            <div>
               <div className="text-xs text-gray-500 uppercase font-semibold">Traceability Health</div>
               <div className="mt-2 flex items-center justify-between">
                  <span className="text-sm text-gray-600">Orphaned Reqs</span>
...
export const Dashboard: React.FC<DashboardProps> = ({ project }) => {
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500 h-full overflow-y-auto bg-gray-50">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Project Dashboard</h2>
          <p className="text-gray-500">Real-time overview of {project.name}</p>
        </div>
        <div className="flex gap-2 mt-4 md:mt-0">
           <button className="text-sm px-3 py-2 bg-white border border-gray-200 rounded-lg text-gray-600 hover:text-primary-600 hover:border-primary-200 transition-colors flex items-center shadow-sm">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="bg-gray-100 p-1 rounded-lg flex text-sm font-medium">
             <button 
                ...
                className={`px-3 py-1.5 rounded-md transition-all flex items-center ${graphMode === 'network' ? 'bg-white text-primary-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
             >
               <Share2 size={14} className="mr-1.5"/> Network
             </button>
             <button 
                onClick={() => setGraphMode('tree')}
                className={`px-3 py-1.5 rounded-md transition-all flex items-center ${graphMode === 'tree' ? 'bg-white text-primary-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
             >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pb-10">
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
          <h3 className="text-lg font-bold text-gray-800 mb-6">Traceability Coverage (V-Model)</h3>
          <div className="h-64">
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
          <h3 className="text-lg font-bold text-gray-800 mb-6">Defect vs Test Progress</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
