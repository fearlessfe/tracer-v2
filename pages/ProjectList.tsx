    ...
    <div className="min-h-screen bg-slate-50 p-8 overflow-y-auto">
      <header className="max-w-7xl mx-auto mb-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage your automotive software lifecycle projects.</p>
        </div>
        <button 
          onClick={openNewProjectModal}
          className="bg-primary-600 hover:bg-primary-700 text-white px-6 py-3 rounded-lg flex items-center shadow-lg shadow-blue-200 transition-all active:scale-95 font-medium"
        >
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((project) => (
          <div 
            ...
            className="group bg-white rounded-xl border border-slate-200 p-6 shadow-sm hover:shadow-xl hover:border-primary-200 transition-all duration-300 cursor-pointer relative overflow-hidden"
          >
            ...
            <div className="mb-2">
               <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-full ${project.type === 'KnowledgeBase' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>
                  {project.type === 'KnowledgeBase' ? 'Knowledge Base' : 'Common Project'}
               </span>
            </div>

            <h3 className="text-xl font-bold text-slate-800 mb-2 group-hover:text-primary-600 transition-colors">{project.name}</h3>
            <p className="text-slate-500 text-sm mb-6 line-clamp-2 h-10">{project.description}</p>

            <div className="flex items-center justify-between border-t border-slate-100 pt-4">
              <div className="flex -space-x-2">
                {project.members.slice(0, 3).map((member, i) => (
                  <div key={i} className="w-8 h-8 rounded-full bg-slate-200 border-2 border-white flex items-center justify-center text-xs font-bold text-slate-600" title={member}>
                    {member.charAt(0)}
                  </div>
                ))}
        ...
        {projects.length === 0 && (
          <div className="col-span-full py-20 text-center text-slate-400 bg-white rounded-xl border-dashed border-2 border-slate-200">
        ...
        <form onSubmit={handleSaveProject} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <select
                ...
                className="w-full px-4 py-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition-all appearance-none bg-white"
              >
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full px-4 py-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition-all h-24 resize-none"
              placeholder="Brief description of the project scope..."
            />
          </div>
          
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Project Members (Multi-select)</label>
            <div className="relative" ref={dropdownRef}>
              <div 
                onClick={() => setIsMemberDropdownOpen(!isMemberDropdownOpen)}
                className="w-full px-4 py-2 rounded-lg border border-slate-300 bg-white cursor-pointer flex justify-between items-center hover:border-primary-400 transition-colors"
              >
                <span className={selectedMembers.length === 0 ? "text-slate-400" : "text-slate-800"}>
              ...
              {isMemberDropdownOpen && (
                <div className="absolute z-10 w-full mt-1 bg-white border border-slate-200 rounded-lg shadow-xl max-h-48 overflow-y-auto">
                  {MEMBER_OPTIONS.map(member => (
                    ...
                    >
                      <span className={`text-sm ${selectedMembers.includes(member) ? 'text-primary-700 font-medium' : 'text-slate-600'}`}>
                        {member}
                      </span>
